      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {data.games.map(game => (
          <Link 
            key={game.id} 
            to={`/game/${game.id}`}
            className="group block border border-gray-200 bg-white hover:border-gray-900 hover:shadow-[4px_4px_0_0_#111827] transition-all p-5"
          >
            <div className="flex justify-between items-start mb-4">
              <h2 className="text-xl font-bold text-gray-900 group-hover:underline decoration-2 underline-offset-4">
                {game.title}
              </h2>
              <span className="p-1 rounded bg-gray-100 text-gray-600 group-hover:bg-gray-900 group-hover:text-white transition-colors">
